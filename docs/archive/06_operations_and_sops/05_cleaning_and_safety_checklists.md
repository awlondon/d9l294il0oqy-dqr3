# Cleaning & Safety Checklists  
Co.work.PLAY Cafe

> This document defines **who does what**, **how often**, and **what counts as “done”**  
> for cleaning and safety procedures across the space.

---

## 1. Zones Covered

- Entry / Host & Retail
- Cafe Bar & Cafe Seating
- Cowork Areas (Open + Quiet + Phone Rooms)
- Main Toddler Play
- Baby / Under-2 & Sensory Room
- Multi-Use / Party Room
- Restrooms
- Back-of-House / Kitchen / Storage
- Exterior Entry (immediate frontage)

Each checklist indicates: **Responsible Role** + **Frequency**.

---

## 2. Daily Opening Checklist (Before First Guests)

### 2.1 Entry / Host & Retail  
**Responsible:** Host + Support  
**Frequency:** Daily, before opening

- [ ] Sweep/vacuum entry floor.
- [ ] Wipe host stand (disinfect high-touch areas: counter, card reader, pens).
- [ ] Clean glass doors and sidelights (fingerprints, smudges).
- [ ] Ensure retail shelves are tidy, fronted, and dust-free.
- [ ] Confirm stroller parking zone is clear and marked.
- [ ] Check for tripping hazards (mats flat, cables secured).
- [ ] Confirm emergency exits and exit paths are unobstructed.

### 2.2 Cafe Bar & Seating  
**Responsible:** Cafe Team  
**Frequency:** Daily, before opening

- [ ] Sanitize all bar surfaces and counters.
- [ ] Wipe cafe tables and chairs (tops and arms).
- [ ] Check chairs and stools for stability.
- [ ] Clean pastry case glass and interior surfaces (as allowed).
- [ ] Sweep/mop cafe floor as needed.
- [ ] Confirm trash cans are empty, liners replaced.
- [ ] Check espresso machine, grinders, and brewer exteriors are clean.

### 2.3 Cowork Areas (Open & Quiet)  
**Responsible:** Host or Assigned Floor Support  
**Frequency:** Daily, before opening

- [ ] Wipe cowork tables and desk surfaces.
- [ ] Wipe chair arms and backs.
- [ ] Check power strips/outlets for visible damage.
- [ ] Remove abandoned items; set aside in lost & found.
- [ ] Confirm phone rooms are clean and ventilated (door open temporarily).

### 2.4 Main Toddler Play  
**Responsible:** Play Attendant  
**Frequency:** Daily, before opening

- [ ] Walk the entire play structure; look for loose bolts, sharp edges, or broken pieces.
- [ ] Check matting and padding; ensure no tears or areas peeling up.
- [ ] Inspect toys for breakage; pull any broken items out of circulation.
- [ ] Wipe high-touch surfaces (rails, handles, slides) with approved disinfectant.
- [ ] Ensure all gates and barriers latch and unlatch correctly.
- [ ] Check posted rules and signage are visible and intact.

### 2.5 Baby / Under-2 & Sensory Room  
**Responsible:** Play Attendant  
**Frequency:** Daily, before opening

- [ ] Inspect soft mats and cushions; ensure they are clean and properly placed.
- [ ] Wipe sensory toys and equipment per cleaning schedule.
- [ ] Confirm rockers/gliders are stable and in good condition.
- [ ] Remove any small-choking-hazard items inappropriate for under-2s.

### 2.6 Multi-Use / Party Room  
**Responsible:** Host or Event Lead  
**Frequency:** Daily, before opening

- [ ] Reset room to default configuration (tables/chairs).
- [ ] Wipe tables and chairs.
- [ ] Sweep/mop as needed.
- [ ] Check AV equipment and cords are safely placed or stored.

### 2.7 Restrooms  
**Responsible:** Assigned Cleaner / Support  
**Frequency:** Daily, before opening (and throughout the day)

- [ ] Clean and disinfect toilets and urinals.
- [ ] Wipe sinks, faucets, and counters.
- [ ] Refill soap, paper towels, toilet paper.
- [ ] Empty trash and replace liners.
- [ ] Mop floor as needed; check for leaks or standing water.
- [ ] Confirm baby changing stations are clean and stocked.

### 2.8 Back-of-House / Kitchen / Storage  
**Responsible:** Cafe Team + Manager  
**Frequency:** Daily, before opening

- [ ] Sanitize food prep surfaces.
- [ ] Check refrigerator and freezer temps; log as required.
- [ ] Ensure chemicals are stored properly and labeled.
- [ ] Confirm storage shelves are organized and off the floor as required by health code.

### 2.9 Exterior Entry (Immediate Area)  
**Responsible:** Host or Assigned Support  
**Frequency:** Daily, before opening

- [ ] Check for trash, debris, and spills.
- [ ] Sweep immediate entry threshold.
- [ ] Confirm external signage and sandwich boards are clean and properly placed (if used).

---

## 3. During-the-Day Cleaning Checklist

### 3.1 Hourly or As-Needed Touch-Ups  
**Responsible:** All roles, led by Manager

- [ ] Wipe cafe tables between guest turnovers.
- [ ] Spot-clean spills immediately (use wet floor signs if needed).
- [ ] Empty trash if more than ~75% full.
- [ ] Quick restroom check every 60–90 minutes:
  - [ ] No major messes, supplies stocked.

### 3.2 Play Area Midday Reset  
**Responsible:** Play Attendant  
**Frequency:** At least once mid-shift; more during peak

- [ ] Return toys to designated zones (blocks, pretend kitchen, books).
- [ ] Quickly wipe heavily used items (per “quick clean” protocol).
- [ ] Remove any toys mouthed excessively or contaminated (set aside for deep clean).
- [ ] Re-check for hazards (broken toys, loose pieces, wet spots on floor).

---

## 4. Closing Checklist (End of Day)

### 4.1 Entry & Host  
- [ ] Remove trash; replace liners.
- [ ] Wipe host stand, door handles, and high-touch surfaces.
- [ ] Tidy retail shelves; consolidate products.
- [ ] Turn off any entry displays, lights per policy.

### 4.2 Cafe Bar & Seating  
- [ ] Fully clean espresso machine per procedure.
- [ ] Wipe all bar surfaces and equipment exteriors.
- [ ] Clean and sanitize cafe tables and chairs.
- [ ] Sweep and mop cafe floor.
- [ ] Discard or store food items according to food safety rules.
- [ ] Remove trash and recycling; clean bins as needed.

### 4.3 Cowork Areas  
- [ ] Wipe all tables and desks.
- [ ] Wipe chair arms and backs.
- [ ] Organize chairs and power cords.
- [ ] Light vacuum or sweep.

### 4.4 Main Toddler Play & Baby Room  
- [ ] Collect and sort toys; separate “needs deep clean” items.
- [ ] Wipe high-touch surfaces (rails, handles, shared toys) with disinfectant.
- [ ] Spot mop any soiled areas on mats or floor.
- [ ] Confirm gates are secured for the night.

### 4.5 Multi-Use / Party Room  
- [ ] Clear tables and surfaces.
- [ ] Wipe tables and chairs.
- [ ] Sweep/mop floor.
- [ ] Reset to default or next-day configuration.

### 4.6 Restrooms  
- [ ] Full clean (toilets, sinks, counters, mirrors).
- [ ] Restock supplies.
- [ ] Mop floors.
- [ ] Empty all trash.

### 4.7 BOH / Kitchen / Storage  
- [ ] Clean prep surfaces and sinks.
- [ ] Sweep and mop floors.
- [ ] Secure food storage areas.
- [ ] Store cleaning chemicals safely.

---

## 5. Weekly / Deep Cleaning Tasks

**Responsible:** Manager to schedule; may be external cleaning vendor or staff.

- [ ] Deep clean play equipment (per manufacturer guidelines).  
- [ ] Launder or deep clean soft play items and cushions.  
- [ ] Dust vents, high surfaces, and light fixtures.  
- [ ] Move furniture and clean underneath in cafe and cowork.  
- [ ] Deep descale espresso machine and coffee equipment (if not done more frequently).
- [ ] Inspect walls and corners for scuffs; touch up as needed.

---

## 6. Printable Two-Sheet Checklists (Copy/Paste Friendly)

Use these two sheets in Word/Docs and export as a 2-page PDF for daily use.

### Sheet 1 – Opening & Closing Checklist

**Title:**

Co.work.PLAY Cafe – Daily Cleaning & Safety Checklist (Opening & Closing)  
Date: ________ Manager: ________

#### A. Opening – Before First Guests

**Entry / Host & Retail**

- [ ] Floors swept/vacuumed; mats flat, no trip hazards
- [ ] Glass doors/sidelights cleaned
- [ ] Host stand sanitized (counter, card reader, pens)
- [ ] Retail shelves tidy, dust-free
- [ ] Stroller parking clearly marked and unobstructed
- [ ] Exits and exit paths clear

**Cafe Bar & Seating**

- [ ] Bar surfaces sanitized
- [ ] Cafe tables and chairs wiped
- [ ] Pastry case glass cleaned; items displayed per policy
- [ ] Trash bins empty; liners replaced
- [ ] Espresso machine, grinders, brewers wiped and ready

**Cowork Areas (Open + Quiet + Phone Rooms)**

- [ ] Tables and desks wiped
- [ ] Chairs checked (no damage; wiped as needed)
- [ ] Phone rooms aired out, surfaces wiped
- [ ] Cables/power strips safe; no trip hazards

**Main Toddler Play**

- [ ] Play structure inspected for loose parts/sharp edges
- [ ] Mats aligned and secure; no tears or lifting edges
- [ ] High-touch surfaces (rails, slides) wiped
- [ ] Broken or suspicious toys removed

**Baby / Under-2 & Sensory Room**

- [ ] Soft mats clean and placed correctly
- [ ] Sensory toys/equipment wiped (per schedule)
- [ ] Rockers/gliders stable, no damage
- [ ] No small choking hazards present

**Multi-Use / Party Room**

- [ ] Default layout set (tables/chairs)
- [ ] Surfaces wiped
- [ ] Floor swept/mopped if needed

**Restrooms**

- [ ] Toilets/urinals cleaned and disinfected
- [ ] Sinks, counters, and faucets wiped
- [ ] Soap, towels, and toilet paper stocked
- [ ] Floors safe and dry

**Back-of-House / Kitchen / Storage**

- [ ] Prep surfaces sanitized
- [ ] Fridge/freezer temps checked & logged (if required)
- [ ] Chemicals labeled and stored safely
- [ ] Storage organized; items off floor as required

**Exterior Entry**

- [ ] Entry threshold and immediate sidewalk swept
- [ ] Trash/debris removed
- [ ] Exterior signage clean and placed correctly

Opening completed by: _______ Time: _______

#### B. Closing – End of Day

**Entry / Host & Retail**

- [ ] Host stand wiped and organized
- [ ] Door handles cleaned
- [ ] Retail shelves neat; items consolidated
- [ ] Trash removed; liners replaced

**Cafe Bar & Seating**

- [ ] Espresso machine cleaned per closing procedure
- [ ] Brewers, grinders, and bar surfaces sanitized
- [ ] Cafe tables and chairs fully wiped
- [ ] Cafe floors swept and mopped
- [ ] Perishable items stored or discarded per policy
- [ ] Trash/recycling emptied; bins wiped if needed

**Cowork Areas**

- [ ] Tables and desks wiped
- [ ] Chairs pushed in and checked
- [ ] Floors vacuumed/swept as needed

**Main Toddler Play & Baby Room**

- [ ] Toys sorted; heavily mouthed/soiled toys set aside for deep clean
- [ ] High-touch play surfaces disinfected
- [ ] Mats/floors spot-mopped where needed
- [ ] Gates/barriers secured

**Multi-Use / Party Room**

- [ ] Party/event items cleared (decor, food, trash)
- [ ] Tables and chairs wiped
- [ ] Floors swept and mopped
- [ ] Room set for next day or first event

**Restrooms**

- [ ] Toilets/urinals cleaned and disinfected
- [ ] Sinks, counters, mirrors wiped
- [ ] Floors mopped
- [ ] All supplies fully restocked
- [ ] Trash removed

**Back-of-House / Kitchen / Storage**

- [ ] Prep surfaces and sinks cleaned and sanitized
- [ ] Floors swept/mopped
- [ ] Food stored/covered/labeled correctly
- [ ] Chemicals locked or safely stored

**Final Manager Safety Walk**

- [ ] All children and adults checked out
- [ ] All exits clear; doors secured
- [ ] Lights and equipment off per closing list
- [ ] Alarm set / security system armed

Closing completed by: _______ Time: _______

### Sheet 2 – Midday & Weekly / Safety

**Title:**

Co.work.PLAY Cafe – Midday & Weekly Cleaning / Safety Checklist  
Week of: ________ Manager: ________

#### A. Midday – During Service (Repeat Throughout Day)

**General (All Staff)**

- [ ] Spills cleaned immediately; wet floor sign used if needed
- [ ] Trash & recycling emptied before overflowing (~75% full)

**Cafe & Cowork**

- [ ] Tables wiped between guests
- [ ] High-crumb or sticky spots cleaned promptly
- [ ] Floor spot-swept in heavy-traffic areas

**Play Areas (Main + Baby)**

- [ ] Toys and props returned to correct zones
- [ ] Heavily used toys quickly wiped (per “quick clean” protocol)
- [ ] Broken/unsafe toys removed from circulation

**Restrooms**

- [ ] Check every 60–90 minutes
- [ ] Major messes cleaned
- [ ] Soap, towels, TP topped up if low

Midday checks done by: ______ (initials/time)

#### B. Weekly / Deep Cleaning Tasks

*(Manager schedules; can be checked off any day of the week.)*

- [ ] Deep clean play equipment (per manufacturer)
- [ ] Launder/deep clean soft items (cushions, fabric toys, etc.)
- [ ] Dust high surfaces, vents, and light fixtures
- [ ] Move furniture and clean underneath (cafe + cowork)
- [ ] Descale espresso machine and coffee equipment (if not more frequent)
- [ ] Inspect walls/corners and address scuffs/marks

#### C. Weekly Safety & Compliance Check

- [ ] Emergency exits and hardware inspected
- [ ] Emergency lights/functioning indicators checked (if applicable)
- [ ] Fire extinguishers in place and inspection tags up to date
- [ ] Play structure fasteners inspected (no looseness or damage)
- [ ] Floors checked for persistent trip hazards (rugs, thresholds, cords)
- [ ] Incident log reviewed for patterns (e.g., repeat locations or issues)

Manager weekly review complete: ______ Date: ______

---

## 7. Daily Cleaning & Safety Checklist – One Pager

> Copy/paste into a single page; add checkboxes or lines as needed.

Co.work.PLAY Cafe — Daily Cleaning & Safety Checklist

Date: ____________  Open by: ____________  Close by: ____________

### Opening – Before First Guests

**Entry / Host & Retail**
- [ ] Floors swept/vacuumed; mats flat, no trip hazards
- [ ] Glass doors and sidelights cleaned (fingerprints, smudges)
- [ ] Host stand sanitized (counter, card reader, pens)
- [ ] Retail shelves tidy, fronted, and dust-free
- [ ] Stroller parking area clear and marked
- [ ] All exits and exit paths unobstructed

**Cafe Bar & Seating**
- [ ] Bar surfaces sanitized
- [ ] Cafe tables and chairs wiped (tops + arms)
- [ ] Pastry case glass cleaned; food displayed per policy
- [ ] Trash bins empty, liners replaced
- [ ] Espresso machine, grinders, brewers wiped and ready

**Cowork (Open & Quiet)**
- [ ] Tables, desks, and chairs wiped
- [ ] Phone rooms aired out (doors open briefly)
- [ ] Cables and power strips safely placed, no trip hazards
- [ ] Abandoned items removed to lost & found

**Main Toddler Play**
- [ ] Play structure inspected: no loose bolts, sharp edges, or broken pieces
- [ ] Mats secure, no tears or peeling edges
- [ ] High-touch surfaces wiped (rails, handles, slides)
- [ ] Toys scanned for obvious damage; broken items pulled

**Baby / Under-2 & Sensory Room**
- [ ] Soft mats and cushions clean and properly placed
- [ ] Sensory toys/equipment wiped as needed
- [ ] Rockers/gliders stable and in good condition
- [ ] No small choking hazards in under-2 area

**Multi-Use / Party Room**
- [ ] Room reset to default layout (tables/chairs)
- [ ] Tables and chairs wiped
- [ ] Floor swept/mopped as needed

**Restrooms**
- [ ] Toilets/urinals cleaned and disinfected
- [ ] Sinks, faucets, and counters wiped
- [ ] Soap, paper towels, and toilet paper stocked
- [ ] Floors dry and safe; trash emptied

**Back-of-House / Kitchen / Storage**
- [ ] Food prep surfaces sanitized
- [ ] Fridge/freezer temps checked and logged (if required)
- [ ] Cleaning chemicals stored safely and labeled
- [ ] Storage shelves organized; items off the floor as required

**Exterior Entry**
- [ ] Entry threshold and immediate sidewalk swept
- [ ] Trash, debris, and spills removed
- [ ] Exterior signage/sandwich boards clean and properly placed

### Midday – During Service (Repeat)

**All Zones**
- [ ] Spills cleaned immediately; wet floor sign used if needed
- [ ] Trash/recycling emptied before overflowing (~75% full)

**Cafe & Cowork**
- [ ] Tables wiped between guest turns
- [ ] High-crumb / sticky areas spot-cleaned

**Play Areas (Main + Baby)**
- [ ] Toys reset into correct zones (blocks, pretend, books, etc.)
- [ ] Heavily used toys and touch points wiped (quick clean)
- [ ] Broken or unsafe toys pulled from circulation

**Restrooms**
- [ ] Quick check every 60–90 minutes
- [ ] Major messes cleaned; supplies topped up if low

### Closing – End of Day

**Entry / Host & Retail**
- [ ] Host stand wiped and organized
- [ ] Glass doors spot-cleaned
- [ ] Retail shelves tidy; products consolidated if needed
- [ ] Trash removed; liners replaced

**Cafe Bar & Seating**
- [ ] Espresso machine cleaned per closing procedure
- [ ] Brewers, grinders, and bar surfaces wiped and sanitized
- [ ] Cafe tables and chairs fully wiped
- [ ] Cafe floor swept and mopped
- [ ] Perishable items discarded or stored per food safety rules
- [ ] Trash and recycling emptied; bins wiped if needed

**Cowork Areas**
- [ ] Tables, desks, and chairs wiped
- [ ] Chairs pushed in; cables neat
- [ ] Floors swept/vacuumed as needed

**Main Toddler Play & Baby Room**
- [ ] Toys sorted; “needs deep clean” items set aside
- [ ] High-touch surfaces disinfected (rails, slides, handles)
- [ ] Mats and floors spot-mopped where needed
- [ ] Gates/barriers secured for the night

**Multi-Use / Party Room**
- [ ] Event items cleared (decor, food, trash)
- [ ] Tables/chairs wiped
- [ ] Floors swept and mopped
- [ ] Room reset for next day (default or next booking)

**Restrooms**
- [ ] Toilets/urinals cleaned and disinfected
- [ ] Sinks, counters, and mirrors wiped
- [ ] Floors mopped
- [ ] Supplies fully restocked
- [ ] Trash emptied

**Back-of-House / Kitchen / Storage**
- [ ] Prep surfaces and sinks cleaned and sanitized
- [ ] Floors swept/mopped
- [ ] Food stored and labeled per policy; nothing left out improperly
- [ ] Cleaning chemicals locked or stored safely

**Final Safety Walk (Manager)**
- [ ] All children and adults checked out
- [ ] All exits clear and doors secured
- [ ] Lights, equipment, and alarms set per closing checklist

---

## 8. Safety Checks (Non-Cleaning)

### 7.1 Weekly Safety Checklist

- [ ] Check all emergency exits and hardware.  
- [ ] Test emergency lighting (if applicable).  
- [ ] Confirm fire extinguishers are in place and inspected (check tags).  
- [ ] Inspect play structure fasteners and joints.  
- [ ] Check for trip hazards (frayed rugs, loose thresholds, loose cables).  
- [ ] Review incident log for patterns and possible mitigations.

### 7.2 Monthly Manager Review

- [ ] Review cleaning logs for completeness.  
- [ ] Spot-audit actual cleanliness vs checklists.  
- [ ] Identify recurring issues and adjust staffing or process.  

---

## 9. Documentation & Logs

- Maintain **daily cleaning checklist sheets** (paper or digital) with:
  - Date, time, initials.
- Maintain **incident and safety log** for:
  - Injuries, near-misses, and notable safety issues.
- Store logs per policy for required retention period.

This document is a template and should be customized to local health and safety codes.
